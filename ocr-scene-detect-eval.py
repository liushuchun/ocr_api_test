...
def recog_scene(access_key, secret_key,url):
    req_url = 'http://argus.atlab.ai/v1/ocr/scene'
    data = {'data': {"uri": url}}
...
def validate_data(gtFilePath, submFilePath, evaluationParams):
    ...
    """
    gt = rrc_evaluation_funcs.load_gt_res_json(gtFilePath, False)

    subm = rrc_evaluation_funcs.load_gt_res_json(submFilePath, True)
    ...
    with open(gt)as fr,open("detect_scene_model.json","w")as fw1:
        for line in fr.readlines():
            ...
            try:
                ...
                for i in range(len(result['result']['text'])):
                    mydict = dict()
                    line = result['result']['bboxes'][i]
                    mydict["bboxes"] = line
                    mydict["text"] = ""
...
def main():
    ...
    result = rrc_evaluation_funcs.main_evaluation(None, default_evaluation_params, validate_data, evaluate_method,args.gt)
